...
void BasicInferenceTool::initialize(const std::vector<FeatureConfig>& features) {
    for (const auto& cfg : features) {
        if (cfg.type == "float") {
...
void BasicInferenceTool::execute(const xAODParticle& p) {
    OutputDataMap out;
    ...
    if (out.count("nVertices")) score -= std::get<IntVec>(out["nVertices"].second)[0];
